...
function HowItWorksCard({ label, value, desc }) {
  return (
    <div className="bg-white border border-gray-100 rounded-sm p-5 shadow-sm text-center">
      <p className="text-xs font-bold tracking-widest uppercase text-dark-600 mb-1">{label}</p>
      <p className="text-2xl font-serif font-bold text-primary mb-2">{value}</p>
  ...
  return (
    <div className="mb-12">
      <div className="flex items-baseline gap-3 mb-4">
        ...
        {section.note && (
          <span className="text-xs text-dark-600 italic">{section.note}</span>
        )}
      </div>
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {HOW_IT_WORKS.map((card) => (
              <HowItWorksCard key={card.label} {...card} />
            ))}
          </div>
          <p className="text-center text-xs text-dark-600 mt-4">*Free delivery until 31st August 2026. Delivery is directly to you.</p>
        </div>
      </section>
      ...
      <section className="py-14 bg-offwhite" aria-label="Order CTA">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="section-title mb-3">Not Sure How Much to Order?</h2>
          <p className="section-subtitle mx-auto mb-7">
            Send us your guest count and the type of event you're hosting, and we'll help you choose the right quantities.
          </p>
          <div className="flex flex-col xs:flex-row gap-3 justify-center">
            <Link to="/contact" className="btn-primary px-8 py-3.5">Make an Enquiry</Link>
            <Link to="/packages" className="btn-outline-dark px-8 py-3.5">View Grazing Tables</Link>
          </div>
        </div>
      </section>
    </>
